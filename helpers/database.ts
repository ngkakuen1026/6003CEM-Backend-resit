...
export const run_insert_multiple = async function run_insert_multiple(sql: string, valuesList: any[][]) {
  try {
    const sequelize = new Sequelize(`postgres://${config.user}:${config.password}@${config.host}:${config.port}/${config.database}`);
    await sequelize.authenticate();
    const data = await Promise.all(valuesList.map(async (values) => {
    ...
    console.error(err, sql, valuesList);
    throw 'Database query error';
  }
}